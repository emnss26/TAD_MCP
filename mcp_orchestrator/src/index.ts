...
server.registerTool(
  "macro_run",
  { title: "Run Macro Plan", description: "Ejecuta un plan de pasos con foreach/template/requires.", inputSchema: MacroRunShape },
  async (args) => {
    // snapshot opcional por si lo usan dentro del plan
    const ctx = await snapshotContext(postRevit, { cacheSec: 30 });
    const result = await runPlan(postRevit, { ...(args as any), vars: { ...(args as any).vars, context: ctx } });
...
server.registerTool(
  "macro_nave_build_basic",
  { title: "Build Basic Nave", description: "Crea ejes, columnas, vigas con pendiente, muros, roof y planos.", inputSchema: NaveShape },
  async (args) => {
    const a = args as any;
    ...
    const result = await runPlan(postRevit, plan as any);
    return asText({ usedContext: { baseLevel: (plan as any).vars.baseLevel, topLevel: (plan as any).vars.topLevel }, result });
  }
);
