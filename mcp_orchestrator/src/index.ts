...
// Aceptamos pasos con `tool` (tu orquestador) o `action` (directo al bridge)
type AnyStep = {
  tool?: string;
  action?: string;
  args?: any;
...
async function runSteps(
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    const action =
      toolToAction[key] ?? (key.includes(".") ? key : undefined); // permite "domain.action" directo
    ...
    try {
      if (action === "__local_context__") {
        const cacheSec =
          typeof s.args?.cacheSec === "number" ? s.args.cacheSec : 30;
...
server.registerTool(
  "orchestrate_plan",
  {
    ...
    description:
      "Devuelve una secuencia de pasos (incluye un snapshot de contexto) para un intent dado.",
...
server.registerTool(
  "macro_run",
  {
    ...
    description:
      "Ejecuta un plan: pásame steps[] o bien intent+args para que yo derive y ejecute.",
